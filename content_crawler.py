...
def rag_web_crawl(query_string):
    ...
    if "leetcode" in query_string.lower():
        ...
        processed_query_for_adapters.append(processed_query_for_adapters)
        print(f"Domain determined: LeetCode. Using query: '{processed_query_for_adapter}' for adapter.")
    else:
        def find_valid_wiki_topics(query_string):
            link = "https://en.wikipedia.org/wiki/?search="+query_string
            search_result = json.loads(search_tool.invoke(link))
            topics = []
            for result in search_result:
                topics.append(result["link"].replace("https://en.wikipedia.org/wiki/", ""))
            return topics

        processed_query_for_adapters = find_valid_wiki_topics(query_string)
        print(f"Domain determined: Wikipedia. Using topic: '{processed_query_for_adapters}' for adapter.")

    t0 = time.time()
    outputs = []
    if domain_to_target == "leetcode":
        print(f"\n--- Fetching LeetCode Data for: '{processed_query_for_adapters}' ---")
        for processed_query_for_adapter in processed_query_for_adapters:
        ...
        for processed_query_for_adapter in processed_query_for_adapters:
            ...
            outputs.append(crawl_result_message)
    return outputs